...
import Sidebar from "../components/Sidebar"; // ❗️ ตรวจสอบ path ให้ถูกต้อง
        ...
        const fetchSchedule = async () => {
            ...
            try {
                const response = await axios.get(
                    "http://localhost:5000/api/exam-plans/", 
                ...
                response.data.forEach(plan => {
                    plan.generated_schedule.forEach(day => {

                        const existingSlots = newMap.get(day.date) || [];
                        newMap.set(day.date, [...existingSlots, ...day.slots]);
                    });
                });
                ...
                setError("ไม่สามารถดึงข้อมูลตารางได้");
            } finally {
                setIsLoading(false);
            }
        };
        ...
        <div className="flex bg-gray-50 min-h-screen">
            ...
            <div className="w-64 bg-white shadow-md">
                ...
                <nav className="mt-6 px-4">
                    ...
                    <button className="mt-4 w-full bg-blue-600 text-white py-2.5 px-4 rounded-lg font-semibold hover:bg-blue-700 transition">
                        Add New +
                    </button>
                </nav>
                                            ...
                                            <div className="mt-1 space-y-1 overflow-y-auto pr-1">
                                                {daySchedule.map((slot, i) => (
                                                    <div 
                                                        ...
                                                        title={`${slot.subject} (${slot.duration} นาที)`}
                                                    >
                                                        {slot.subject}
                                                    </div>
                                                ))}
